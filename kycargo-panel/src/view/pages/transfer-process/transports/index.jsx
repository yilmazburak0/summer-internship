...
import { fetchTransports, setModalType, setSelectedTransport, toggleCreateEditModal } from "redux/transport/transportActions";
import vehicleFields, { getColumns } from "./transportfields";
import TransportCreateEdit from "./modals/TransportCreateEdit";
...
export default function Transports() {
    ...
    const { transports, loading, totalTransportCount } = useSelector(s => s.transport)
    ...
    const getFilteredProducts = (pageIndex) => {
        ...
        dispatch(fetchTransports({ filter }))
    };
                ...
                <Col span={24}>
                    <Table
                        onRow={(record) => {
                            return {
                                onDoubleClick: async () => {
                                    await dispatch(setSelectedTransport(record))
                        ...
                        loading={loading}
                        pagination={{ onChange: onPageChange, current: pageIndex, pageSize, total: totalTransportCount, showSizeChanger: true }}
